...
type WishlistStore = {
  ...
  fetchWishlistItems: (userId: number) => Promise<void>;
  addToWishlist: (
    userId: number,
    type: string,
    itemId: number
  ...
  resetWishlist: () => void;
  syncWishlistWithBackend: (userId: number) => Promise<void>;
};
      ...
      removeFromWishlist: async (itemId) => {
        ...
        try {
          await apiClient.delete(
            `${apiClient.URLS.wishlist}/${itemId}`,
            "",
            true
          );
          set((state) => ({

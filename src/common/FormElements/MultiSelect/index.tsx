      ...
      {({ open }) => (
        <div className="relative">
          {(label && label.trim().length > 0) && <Listbox.Label htmlFor={id} className={twMerge(clsx({
            "mb-2 inline-block text-xs font-Gordita-Medium text-[#333333]": true,
            "cursor-pointer": id
          ...
          }
          {
            errorMsg && errorMsg.trim().length > 0 && (
              <p className={twMerge(clsx({ "text-red-400 text-xs mt-1 font-Gordita-Regular ": true }), errorCls)}>{errorMsg}</p>
            )
          }
        </div>
      )}

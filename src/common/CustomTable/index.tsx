          ...
          <tr className='divide-x divide-x-black w-auto'>
            {
              columns.map((col, index) => {
                ...
                return (
                  <th key={`${index}-${key}`} scope='col' onClick={onClick} className={twMerge("text-base text-left  whitespace-nowrap font-Gordita-Regular px-2 py-1 w-auto", thCls)}>{label}</th>
                )
              })
            }
          </tr>

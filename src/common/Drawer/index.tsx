                ...
                <div className={twMerge("flex h-full w-full gap-5 flex-col px-6 py-6", panelInnerCls)}>
                  {
                    !hideHeader && (
                      <div className={clsx("flex items-start justify-between", (!title || title.length == 0) && "flex-row-reverse")}>
                        {
                          title && (
                            <Dialog.Title className={twMerge("text-[18px] font-Gordita-Bold leading-6 text-gray-900", titleCls)}>
                              {
                                title
                              }
                            </Dialog.Title>
                        ...
                        <VscChromeClose className={twMerge('text-white text-[25px] cursor-pointer', closeIconCls)} onClick={() => handleDrawerToggle(false)} />
                      </div>
                    )
                  }

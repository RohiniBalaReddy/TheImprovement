        ...
        <div className="flex md:flex-row flex-col w-full items-center justify-center md:mb-10 mb-5 md:gap-4 gap-2">
          <div className="relative md:w-[35%] w-full md:h-[550px] h-[200px] md:sticky md:top-0 md:self-start bg-gray-200 rounded-[10px]">
            ...
            <div className="absolute bottom-4 left-4">
              <span className="inline-block bg-white backdrop-blur-md text-gray-900 md:px-4 px-2 py-1 md:py-2 md:rounded-[10px] rounded-[4px] md:text-lg text-[12px] font-Gordita-Bold">
                {style}
              </span>
            </div>
          </div>
          ...
          <div className="md:w-[50%] w-full flex flex-col md:gap-6 gap-3 md:px-6 px-1 justify-center md:mt-0 mt-3">
            <div className="w-full md:text-[12px] text-[10px] mb-4 font-Gordita-Medium text-gray-900">
              <div className="flex md:flex-nowrap flex-wrap items-center space-x-1">
                ...
                <span className="underline underline-offset-2 text-[#5297ff]">
                  <span className="hidden md:inline">{title}</span>
                  <span className="block md:hidden">
                    {title?.length > 10 ? `${title?.slice(0, 10)}...` : title}
                  </span>
            ...
            <div className="flex md:flex-row flex-col gap-2">
              <h1 className="font-Gordita-Medium text-[#5297FF] md:max-w-[70%] md:text-[20px] text-[18px] md:text-left text-center">
                {title}
              </h1>
              <div className="flex md:items-center justify-end gap-2">
                <Button
                  onClick={handleShare}
                  className="flex items-center justify-center md:w-10 w-6 md:h-10 h-6 bg-white hover:text-white hover:bg-[#5297ff]  border border-black rounded-full text-lg transition-colors"
                >
            ...
            <div className="flex flex-col gap-1">
              <div className="bg-blue-50 md:rounded-[10px] rounded-[4px] md:p-5 p-3 shadow-custom">
                <h2 className="font-Gordita-Bold md:text-base text-sm text-blue-800 mb-3 pb-2 border-b border-blue-200">
                  {activetab} Design Details:
                </h2>

                <div className="grid md:grid-cols-2 grid-cols-1 gap-4">
                  <div className="flex flex-col">
                    <span className="font-Gordita-Medium text-[#5297ff] md:text-sm text-xs mb-1">
                      Style
                    </span>
                    <span className="font-Gordita-Regular md:text-base text-sm">
                      {style}
                    </span>
                  </div>
            ...
            <h2 className="text-[#7B7C83] md:text-[14px] text-[12px] font-Gordita-Medium md:leading-6 leading-3 md:text-left text-center">
              {description}
            </h2>
            <div className="md:pb-0 pb-1 md:px-0 px-4">
              <Button
                className="md:w-[50%] w-full md:rounded-[8px] rounded-[4px] bg-[#5297FF]  p-2 md:text-[20px] font-Gordita-Medium text-[12px] text-white md:static fixed bottom-0  right-0 "
                  ...
                  <div className="flex flex-col md:gap-y-[48px] gap-y-[24px] p-5">
                    <div className="grid grid-cols-1 md:gap-5 gap-2">
                      <CustomInput
                        ...
                        name="name"
                        placeholder="Name"
                        className="md:h-[30px] h-[20px] px-2 py-1 border w-[368px] border-[#A1A0A0] rounded-[4px]"
                        rootCls="px-2 py-1"
                      />
                      <CustomInput
                        type="email"
                    ...
                    <Button className="bg-[#3586FF] md:px-5 px-3 md:py-4 py-3 md:text-[16px] font-Gordita-Medium text-[12px] rounded-md text-white">
                      Get A Quote
                    </Button>
                  </div>
                </form>

                <p className="text-center font-Gordita-Regular md:text-[16px] text-[10px]">
...
const HighlightsSection = ({
  ...
  <div className="bg-white rounded-xl p-4 shadow-sm">
    <h2 className="font-Gordita-Bold md:text-base text-sm text-[#5297FF] mb-3 flex items-center">
      <div className="w-1.5 h-1.5 bg-[#5297ff] rounded-full mr-2"></div>
      {title}
    </h2>
    <ul className="space-y-2.5">
      {items?.map((item, idx) => (
        <li key={idx} className="flex items-start gap-3">
          <CheckCircle2 className="w-4 h-4 text-[#5297FF] mt-0.5 flex-shrink-0" />
          <span className="font-Gordita-Regular md:text-[12px] text-[10px] text-gray-700">
            {item}
          </span>
        </li>
      ))}

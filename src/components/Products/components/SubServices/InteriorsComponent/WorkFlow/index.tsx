...
const WorkFlow = ({ heading, subHeading, list }: IListSectionProps) => {
  ...
  return (
    <div className="flex flex-col items-center justify-centers ">
      <h1 className="font-Gordita-Medium md:text-[24px] text-[18px] md:mb-3 mb-2">{heading}</h1>
      <h2 className="font-Gordita-Medium text-[#7B7C83] md:text-[20px] text-[14px] text-center ">
        {subHeading}
      </h2>
      <div className="md:flex hidden flex-row flex-wrap md:flex-nowrap items-center justify-center gap-5 mt-[64px]">
        {list.map(({ imageUrl, label }, index) => (
          ...
          >
            ...
            <p
              className={clsx(
                "text-[15px] font-Gordita-Medium text-[#7B7C83] mt-3",
        ...
        <Slider ref={sliderRef} {...sliderSettings}>
          {list.map(({ imageUrl, label }, index) => (
            ...
            >
              ...
              <p
                className={clsx(
                  "text-[12px] font-Gordita-Medium text-[#7B7C83] mt-3",
                  index === 0 ? "font-bold" : "font-normal"
                )}
              >
                {label}
